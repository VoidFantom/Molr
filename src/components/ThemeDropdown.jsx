    ...
    <div className="relative w-full" ref={dropdownRef} onMouseLeave={handleMouseLeave}>
      ...
      >
        <div className="flex items-center gap-3 font-medium">
          <div className="w-5 h-5 rounded-full border shadow-sm" style={{ backgroundColor: activeThemeObj.color, borderColor: 'var(--border-color)' }}></div>
          {activeThemeObj.name}
        </div>
      ...
      {isOpen && (
        <div className="absolute top-full mt-1 w-full rounded-xl shadow-lg border z-50 overflow-hidden dropdown-anim" style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border-color)' }}>
          {themes.map((t) => (
            ...
            >
              <div className="w-5 h-5 rounded-full border shadow-sm" style={{ backgroundColor: t.color, borderColor: 'var(--border-color)' }}></div>
